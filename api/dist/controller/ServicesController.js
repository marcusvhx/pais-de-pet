"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ServicesController = void 0;
const ErrorWithStatus_1 = require("../utils/ErrorWithStatus");
const ServiceModel_1 = __importDefault(require("../models/ServiceModel"));
class ServicesController {
    static create(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const { name, employeesIds } = req.body;
            try {
                const lastService = yield ServiceModel_1.default.findOne().sort({ id: -1 });
                const id = lastService ? lastService.id + 1 : 1;
                const newServices = yield ServiceModel_1.default.create({ id, name, employeesIds });
                res.status(201).json(newServices);
            }
            catch (err) {
                next(err);
            }
        });
    }
    static edit(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const { name, employeesIds } = req.body;
            const { id } = req.params;
            try {
                const editedService = yield ServiceModel_1.default.updateOne({ id }, { name, employeesIds }, { new: true });
                res.status(200).json(editedService);
            }
            catch (err) {
                next(err);
            }
        });
    }
    static getAll(_, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const services = yield ServiceModel_1.default.find();
                res.json(services);
            }
            catch (err) {
                next(err);
            }
        });
    }
    static getById(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            try {
                const service = yield ServiceModel_1.default.findOne({ id });
                if (!service) {
                    throw new ErrorWithStatus_1.ErrorWithStatus("serviços não encontrado", 404);
                }
                res.json(service);
            }
            catch (err) {
                next(err);
            }
        });
    }
    static deleteById(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            try {
                yield ServiceModel_1.default.findOneAndDelete({ id });
                res.json({ msg: "serviço deletado" });
            }
            catch (err) {
                next(err);
            }
        });
    }
}
exports.ServicesController = ServicesController;
